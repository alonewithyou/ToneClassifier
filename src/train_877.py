...
def pre_processf0(feature):
    ...
    y_ax = list(cur)
    #fc = itp.interp1d(x_ax, y_ax, kind='linear')
    #feature = fc(x_ax)
    tck = itp.splrep(x_ax, y_ax, s=None)
    feature = itp.splev(x_ax, tck)
    #print(feature)
    cur = []
    for i in range(New_length):
        po = 1. * i / New_length * (len(feature) - 1)
        p1 = int(po)
        p2 = p1 + 1
...
def regenerate(Y, interval):
    ret = []
    for i in range(0, len(Y), interval >> 1):
        l = i
        r = min(l + interval, len(Y))
        #print(l, r, len(Y))
        x = [j for j in range(l, r)]
        y = Y[l:r]
        ret.append(np.polyfit(x, y, 1)[0])
    return ret
...
def prepare(path):
    ...
    for i in range(len(A)):
        ...
        cut3 = regenerate(list(cut1), Intervals[0])
        cut3 += regenerate(list(cut1), Intervals[1])
        cut3 = fill(cut3)
        cut4 = regenerate(list(cut2), Intervals[0])
        cut4 += regenerate(list(cut2), Intervals[1])
        cut4 = fill(cut4)
    ...
    ret2 = B
    '''
    label_file = open(path + '/labelnew-temp.csv', 'wb')
    f0_file = open(path + '/datanew-temp.csv', 'wb')
    for num in ret1:
