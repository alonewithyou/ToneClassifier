...
def generate_data(sample_size, data, labels, type):
    ...
    X = X.reshape((sample_size, channels, -1))
    return X, Y


def DNN():
    input = C.ops.input_variable((channels, RESIZE), np.float32)
    label = C.ops.input_variable(4, np.float32)

    with C.layers.default_options(pad=True):
        den0 = Dense(1 << 10, activation=C.sigmoid)(input)
        den1 = Dense(1 << 10, activation=C.sigmoid)(den0)
        den2 = Dense(1 << 10, activation=C.sigmoid)(den1)
